[string[]]$array = "d dec 683 if qn == 0
d dec -220 if h == 0
...
h inc -526 if d != -2461" -split "`n";

<# >
[string[]]$array = "b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10" -split "`n";
#>
...
foreach ($line in $array) {
    ...
    $varpos = $var.IndexOf($broken[0])
    $varval = $vv[$varpos]
    $conpos = $var.IndexOf($broken[4])
    $conval = $vv[$conpos]
    
    $n = 0
    switch ($broken[5]) {
        "==" { if ($conval -eq $broken[6]) { $n = 1 } }
        ">"  { if ($conval -gt $broken[6]) { $n = 1 } }
        "<"  { if ($conval -lt $broken[6]) { $n = 1 } }
        ">=" { if ($conval -ge $broken[6]) { $n = 1 } }
        "<=" { if ($conval -le $broken[6]) { $n = 1 } }
        "!=" { if ($conval -ne $broken[6]) { $n = 1 } }
        default { "UNKNOWN OPERATOR ->",$broken[5],"<-" }
    }
    if ($n -eq 1) {
        Switch ($broken[1]) {
            "inc" { $varval += $broken[2] }
